...
void InvocationGUIWidget::parseDescriptor(QJsonObject *invocationJSON)
{
    ...
    for (auto& idAndInputObject: idToInputObject)
    {
        ...
        {
            widget = inputType == "File" ? new DropWidget() : new QWidget();        // File input are droppable, meaning the user can drop a widget on them (use in medInria to drop data taken from the database)
...
bool InvocationGUIWidget::mustGenerateCompleteInvocation()
{
    return this->optionalInputGroup != nullptr && this->optionalInputGroup->isChecked();
}

void InvocationGUIWidget::populateDirectoriesAndSetOutputFileName(QJsonObject &invocationJSON, QStringList &directories)
{
    ...
    // See description in the header at InvocationGUIWidget::populateDirectoriesAndSetOutputFileName()
...
QString InvocationGUIWidget::getOutputFileName()
{
...
void InvocationGUIWidget::populateOutputDirectoriesAndSetOutputFileName(const QJsonObject &invocationJSON, QStringList &directories, bool &hasChangedCurrentDirectory)
{
...
void InvocationGUIWidget::toggleOutputFile(QPushButton *toggleOutputPushButton, const QString &inputId)
{
